...
const deleteStage = async (req, res) => {
    ...
    doc.save(async err => {
        if(err){
            ...
            await TaskModel.deleteMany({_id: req.session.project, stage: stageTitle});
...
const moveTasks = async (req, res) => {
    ...
    if(req.body.direction === 'forward'){
        return res.status(200).json(await TaskModel.updateMany({_id: req.session.project, stage: stageIndex}, {stage: stageIndex+1}));
    }else{
...
const deleteTasks = async (req, res) => {
    ...
    if(!/^\d+$/.test(req.params.stageIndex)) return res.status(400).json({message: "stageIndex must be a number"});
    const stageIndex = Number(req.params.stageIndex);
    const deleted = await TaskModel.deleteMany({_id: req.session.project, stage: stageTitle});
