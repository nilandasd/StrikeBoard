      ...
      getTasks(props.stage)
        .then(querySnapshot => {
          ...
          querySnapshot.forEach(doc => {
            const id = doc.id;
            const { assigned, description, pid, title, points } = doc.data();
            tasks.push({ assigned, description, pid, title, id, points, stage: props.stage});
          })
          setTasks(tasks);
      ...
      <>
        <div>
          <Card style={{
            ...
            <Card.Body >
              <div style={{marginLeft: "5px", marginRight: "5px", display:"flex", justifyContent:"space-between", borderBottom:"1px solid grey"}}>
                {editingTitle ?
                ...
                <Dropdown >
                  ...
                  <Dropdown.Menu>
                    <Dropdown.Item disabled={props.stage === currentProject.stages.length - 1} onClick={handleMoveAllTasksRight}>Move all tasks to next stage</Dropdown.Item>
                    <Dropdown.Item disabled={props.stage === 0} onClick={handleMoveAllTasksLeft}>Move all tasks to previous stage</Dropdown.Item>
                    <Dropdown.Divider />
                    <Dropdown.Item disabled={loading}
                      style={{ color: '#ca0000' }}
              ...
              <AddTask title={props.title} tasks={tasks} setTasks={setTasks} loading={loading} stage={props.stage}/>  
            </Card.Body>
          </Card>
        </div>
      </>
